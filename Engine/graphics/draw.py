...
def draw_map(map: GameMap, surface: pygame.Surface):
    surface.fill(GREY)
    for tile in map:
        tile.draw(surface, map)


def draw_squads(map: GameMap, surface: pygame.Surface, squads: list[Squad]):
    for squad in squads:
        squad.rect.center = map.get_tile_px_placement(squad.tile_location)
        surface.blit(squad.image, squad.rect)


def draw_cities(map: GameMap, surface: pygame.Surface):
    for city in map.cities:
        city.rect.center = map.get_tile_px_placement(city.tile_location)
        surface.blit(city.image, city.rect)


def highlight_chosen_squad(map: GameMap, surface: pygame.Surface, squad: Squad):
